...
function isAllDatesValid(dateBirth, passportDateIssue) {
    /**
    ...
    if ((checkPassportDateIssue <= checkDateBirth)) {
...
function confirmPassword() {
    ...
    if (document.querySelector('.input-control #password').value == document.querySelector('.input-control #confirm-password').value) {
        document.querySelector('.input-control .span-confirm-psw').style.display = 'none';
    } else {
        document.querySelector('.input-control .span-confirm-psw').style.display = 'inline';
    }
}

var check = false;

async function queryExistingData(email, passport, cpf='') {

    $.ajax({
        async: false,
        method: "POST",
        url: "../assets/php/checkExistingData.php",
        data: {
            'passport': passport,
            'cpf': cpf,
            'email': email
...
document.querySelector('.input-control #confirm-password').addEventListener('keyup', function() {
...
form.addEventListener('submit', (event) => {
    ...
    queryExistingData(inputEmail.value, inputPassport.value, inputCpf.value)
