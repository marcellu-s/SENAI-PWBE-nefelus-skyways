...
<body>
    ...
    <main>
        <section class="choose-your-flight">
            ...
            <form class="cyf-content" action="./selecionar_assento_ida.php" method="post">
            ...
            // INFOS VOO
            $voo_ida = $_POST['flight-selected'];

            $origem_volta = $_POST['going-to'];
            $destino_volta = $_POST['travel-from'];
            $saida = $_POST['outbound'];
            $retorno = $_POST['return'];


            // INFO GERAL PASSAGEIROS
            ...
            echo '<div class="info-passengers">';

            // REPASSAMENTO DAS INFOS DOS PASSAGEIROS PRA SELECIONAR_ASSENTO_IDA.PHP
            for($i = 0; $i < $total_passageiros; $i++) {

                echo '
                    <div class="info-passengers" id="info-passenger-'. $i .'">

                        <input type="hidden" name="first-name-passenger-'.$i.'" value="'. $passageiro[$i]['p_nome'] .'">
                        <input type="hidden" name="last-name-passenger-'.$i.'" value="'. $passageiro[$i]['s_nome'] .'">
                        
                        <input type="hidden" name="date-of-birth-passenger-'.$i.'" value="'. $passageiro[$i]['data_nasc'] .'">
                        <input type="hidden" name="gender-passenger-'.$i.'" value="'. $passageiro[$i]['genero'] .'">

                        <input type="hidden" name="nationality-passenger-'.$i.'" value="'. $passageiro[$i]['nacionalidade'] .'">
                        <input type="hidden" name="passport-passenger-'.$i.'" value="'. $passageiro[$i]['passaporte'] .'">                        

                        <input type="hidden" name="cpf-passenger-'.$i.'" value="'. $passageiro[$i]['cpf'] .'">
                        <input type="hidden" name="no-cpf-passenger-'.$i.'" value="'. $passageiro[$i]['sem_cpf'] .'">
            ...
            // ABAIXO QUERY PARA O VOO DE VOLTA
            include_once "../../ops/db.php";

            $query_id = mysqli_query($conn, "SELECT id_aeroporto FROM aeroporto where nome = '$origem_volta'");
            $id_origem = mysqli_fetch_assoc($query_id); 
            $id_origem_volta = $id_origem['id_aeroporto'];

            $query_id = mysqli_query($conn, "SELECT id_aeroporto FROM aeroporto where nome = '$destino_volta'");
            $id_destino = mysqli_fetch_assoc($query_id); 
            $id_destino_volta = $id_destino['id_aeroporto'];
            ...
            WHERE origem = $id_origem_volta AND destino = $id_destino_volta AND date(data_ida) = '$retorno'";
            ...
            while ($row = mysqli_fetch_assoc($result_voos)) {
                ...
                <div class="flights-wrapper">
                    <div class="flight">
                        <input type="hidden" name="flight-selected" value="'. $voo_ida .'">
                        <input type="radio" name="return-flight-selected" id="" class="input-flight" value="' . $row['id_voo'] . '">
